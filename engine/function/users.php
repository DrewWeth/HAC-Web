...
function fetchImages($status) {
	$status = (int)$status;
	$query = mysql_query("SELECT `id`, `title`, `desc`, `date`, `status`, `image`, `account_id` FROM znote_images WHERE `status`='$status' ORDER BY `date` DESC;");
	$array = array();
...
function fetchMurders() {
	$query = mysql_query("SELECT `killed_by`, COUNT(`killed_by`) AS `kills` FROM `player_deaths` WHERE `is_player`='1' GROUP BY `killed_by` ORDER BY COUNT(`killed_by`) DESC LIMIT 0, 10;");
	$array = array();
	if ($query)
	{
		while($row = mysql_fetch_assoc($query)) {;
...
function fetchLoosers() {
	$query = mysql_query("SELECT `player_id`, (SELECT `name` FROM `players` WHERE `id` = `player_id`) AS `name`, COUNT(`player_id`) AS `Deaths` FROM `player_deaths` GROUP BY `player_id` ORDER BY COUNT(`player_id`) DESC LIMIT 0, 10;");
	$array = array();
	while($row = mysql_fetch_assoc($query)) {;
		unset($row['player_id']);
		$array[] = $row;
...
// Fetch latest deaths
function fetchLatestDeaths() {
	$query = mysql_query("SELECT * FROM `player_deaths` ORDER BY `time` DESC LIMIT 0, 30;");
	$array = array();
	while($row = mysql_fetch_assoc($query)) {
		$data = user_character_data($row['player_id'], 'name');
		$row['victim'] = $data['name'];
		unset($row['player_id']);
		$array[] = $row;
	}
	return !empty($array) ? $array : false;
...
function fetchLatestDeaths_03($rowz = 30, $killers = false) {
	$countz = 0;
	if ($rowz === false || $killers === true) $getdeaths = mysql_query("SELECT * FROM player_deaths ORDER BY date DESC;")or die(mysql_error());
	else $getdeaths = mysql_query("SELECT * FROM player_deaths ORDER BY date DESC LIMIT 0, $rowz;")or die(mysql_error());
    $data = false;
	while ($showdeaths = mysql_fetch_assoc($getdeaths)) {
        $pid = $showdeaths['player_id'];  
        ...
        $getname = mysql_fetch_assoc(mysql_query("SELECT name FROM players WHERE id = '$pid'"));  
		$name = $getname['name'];
		$row = array();
		$row['level'] = $level;
		$row['victim'] = $name;
		$row['time'] = $showdeaths['date'];
...
function getPlayerStorage($player_id, $storage) {
...
function user_is_online($player_id) {
	$status = user_character_data($user_id, 'online');
	if ($status !== false) {
...
function average_guild_level($gid) {
	$gid = (int)$gid;
	return mysql_result(mysql_query("SELECT (SUM(p.level)/COUNT(p.id)) AS 'average' FROM players AS p LEFT JOIN guild_ranks AS gr ON gr.id = p.rank_id WHERE gr.guild_id =$gid"), 0, 'average');
}
...
function get_guild_wars() {
	$query = mysql_query("SELECT `id`, `guild1`, `guild2`, `name1`, `name2`, `status`, `started`, `ended` FROM `guild_wars` ORDER BY `started` DESC LIMIT 0, 30");
	$array = array();
	while($row = mysql_fetch_assoc($query)) {
		$array[] = $row;
	}
...
function user_account_id_from_password($password) {
	...
	return mysql_result(mysql_query("SELECT `id` FROM `accounts` WHERE `password`='".$password."';"), 0, 'id');
}
...
function user_id($username) {
	$username = sanitize($username);
	return mysql_result(mysql_query("SELECT `id` FROM `accounts` WHERE `name`='$username';"), 0, 'id');
}
